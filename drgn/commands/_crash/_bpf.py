...
@crash_command(
    ...
    arguments=(
        ...
        argument(
            ...
            action="store_true",
            default=False,
            help="display more details for every BPF program",
        ),
        argument(
            "-m",
            dest="map_id",
        ...
        argument(
            ...
            action="store_true",
            default=False,
            help="display more details for every BPF map",
        ),
        argument(
            "-s",
            dest="show_struct",
            action="store_true",
            default=False,
            help="display the full struct bpf_map (with -m or -M) or struct bpf_prog and "
            "struct bpf_prog_aux (with -p or -P); combine with -x or -d to control "
...
for bpf_map in bpf_map_for_each(prog):
    ...
    if args.show_struct and not (
        args.prog_id is not None
        or args.show_all_progs
        or args.map_id is not None
        or args.show_all_maps
    ):
        raise CommandArgumentError("-s option requires either -p, -P, -m or -M")

    has_printed_details = False
    ...
    if args.prog_id is not None:
        ...
        has_printed_details = True

    if args.show_all_progs:
        ...
        for bpf_prog in bpf_prog_for_each(prog):
            ...
            aux = bpf_prog.aux
            prog_id = aux.id.value_()
            prog_type_name = bpf_prog.type.format_(type_name=False).split(
                "BPF_PROG_TYPE_"
            )[-1]
            INSN_SIZE = prog.type("struct bpf_insn").size
            ...
            uid = kuid_val(aux.user.uid)
            try:
                prog_name = escape_ascii_string(
                    aux.name.string_(), escape_backslash=True
                )
                if not prog_name:
                    prog_name = "(unused)"
            except AttributeError:
    ...
    # Default display (neither specific IDs nor all programs/maps details requested)
